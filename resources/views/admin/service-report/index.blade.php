  ...
  <main id="main-content" class="flex-1 md:ml-64 pt-16 flex flex-col h-screen overflow-x-hidden relative transition-all duration-300">
    <div class="flex-1 overflow-x-hidden overflow-y-auto p-4 md:p-8 relative w-full">
        ...
        <div class="max-w-7xl mx-auto relative z-10">
          ...
          <!-- Search & Filter Card -->
          <form action="{{ route('service-report.index') }}" method="GET" class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row md:flex-wrap items-center gap-4 mb-6">
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="p-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('service-report.print', $report->id) }}" target="_blank" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded" title="Cetak Service Report">
                                    ...
                                    <form action="{{ route('service-report.destroy', $report->id) }}" method="POST" id="delete-form-{{ $report->id }}" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="confirmDelete({{ $report->id }})" class="p-1.5 text-red-600 hover:bg-red-50 rounded" title="Hapus">
