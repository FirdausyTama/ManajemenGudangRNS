...
<!-- Modal Edit SPH -->
<div id="editModal" class="fixed inset-0 bg-gray-900/60 hidden z-[60] overflow-y-auto backdrop-blur-sm">
    <div class="flex min-h-full items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-5xl my-8 transform transition-all overflow-hidden flex flex-col border border-gray-200" id="edit-modal-content">
...
<script>
    ...
    // Numerical to Terbilang function in JS
    function getTerbilang(nilai) {
        nilai = Math.abs(nilai);
        const huruf = ["", "Satu", "Dua", "Tiga", "Empat", "Lima", "Enam", "Tujuh", "Delapan", "Sembilan", "Sepuluh", "Sebelas"];
        let temp = "";
        if (nilai < 12) { temp = " " + huruf[nilai]; }
        else if (nilai < 20) { temp = getTerbilang(nilai - 10) + " Belas"; }
        else if (nilai < 100) { temp = getTerbilang(Math.floor(nilai / 10)) + " Puluh" + getTerbilang(nilai % 10); }
        else if (nilai < 200) { temp = " Seratus" + getTerbilang(nilai - 100); }
        else if (nilai < 1000) { temp = getTerbilang(Math.floor(nilai / 100)) + " Ratus" + getTerbilang(nilai % 100); }
        else if (nilai < 2000) { temp = " Seribu" + getTerbilang(nilai - 1000); }
        else if (nilai < 1000000) { temp = getTerbilang(Math.floor(nilai / 1000)) + " Ribu" + getTerbilang(nilai % 1000); }
        else if (nilai < 1000000000) { temp = getTerbilang(Math.floor(nilai / 1000000)) + " Juta" + getTerbilang(nilai % 1000000); }
        return temp.trim();
    }
    ...
    function calculateTotal() {
        ...
        $('#add-total-display').text(formatted);
        $('#add-terbilang-display').text(getTerbilang(total) + " Rupiah");
    }

    $(document).on('input', 'input[name*="[kuantitas]"], input[name*="[harga_satuan]"]', calculateTotal);

    $('#add-row-btn').on('click', function() {
        const row = `
            <tr class="item-row">
                <td class="border border-gray-800 px-2 py-1 text-center align-top pt-2">${addIndex + 1}</td>
                <td class="border border-gray-800 px-2 py-1 align-top">
                    <select name="items[${addIndex}][barang_id]" onchange="updateItemData(this)" class="w-full border border-blue-200 rounded px-2 py-0.5 mb-1 text-xs focus:ring-1 focus:ring-rns-blue outline-none bg-blue-50/50">
                        <option value="">-- Pilih dari Stok --</option>
                        @foreach($barangs as $b)
                            <option value="{{ $b->id }}">{{ $b->name }}</option>
                        @endforeach
                    </select>
                    <input type="text" name="items[${addIndex}][nama_barang]" required class="w-full font-bold outline-none mb-1 border-b border-gray-100 text-sm" placeholder="Nama Barang">
                    <textarea name="items[${addIndex}][spesifikasi]" rows="2" class="w-full text-xs text-gray-600 outline-none bg-transparent resize-none border-b border-gray-50 mb-1" placeholder="Spesifikasi..."></textarea>
                    <div class="mt-1">
                        <label class="block text-[9px] uppercase font-bold text-gray-400 mb-0.5">Upload Foto Produk (Opsional)</label>
                        <input type="file" name="items[${addIndex}][images][]" multiple accept="image/*" class="w-full text-[10px] text-gray-500 file:mr-2 file:py-0.5 file:px-2 file:rounded file:border-0 file:text-[10px] file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                    </div>
                </td>
                <td class="border border-gray-800 px-2 py-1 align-top pt-2">
                    <div class="flex items-center gap-1">
                        <input type="number" name="items[${addIndex}][kuantitas]" value="1" step="0.01" required class="w-full text-center outline-none bg-transparent font-medium border-b border-gray-100 text-sm">
                        <input type="text" name="items[${addIndex}][satuan]" value="Unit" required class="w-12 text-center outline-none bg-transparent text-xs border-b border-gray-100">
                    </div>
                </td>
                <td class="border border-gray-800 px-2 py-1 align-top text-right pt-2">
                    <input type="number" name="items[${addIndex}][harga_satuan]" required class="w-full text-right outline-none bg-transparent font-bold border-b border-gray-100 text-sm">
                </td>
                <td class="border border-gray-800 px-1 py-1 text-center align-top pt-2">
        ...
        `;
        $('#add-items-tbody').append(row);
        addIndex++;
        $('.remove-item').prop('disabled', false).removeClass('text-red-300 cursor-not-allowed').addClass('text-red-500');
    });
    ...
    $(document).on('click', '.remove-item', function() {
        if ($('.item-row').length > 1) {
            ...
            calculateTotal();
            if ($('.item-row').length === 1) {
                $('.remove-item').prop('disabled', true).addClass('text-red-300 cursor-not-allowed');
            }
        }
    });
    ...
    // AJAX EDIT Logic
    function editSPH(id) {
        $('#edit-modal-content').html(`<div class="p-20 text-center"><div class="animate-spin inline-block w-8 h-8 border-4 border-rns-blue rounded-full border-t-transparent"></div></div>`);
        openModal('editModal');

        $.get(`/admin/surat-penawaran/${id}/edit-data`, function(data) {
            let itemsHtml = '';
            data.items.forEach((item, index) => {
                itemsHtml += `
                    <tr class="item-row-edit">
                        <td class="border border-gray-800 px-2 py-1 text-center align-top pt-2">${index + 1}</td>
                        <td class="border border-gray-800 px-2 py-1 align-top">
                            <input type="hidden" name="items[${index}][barang_id]" value="${item.barang_id || ''}">
                            <input type="text" name="items[${index}][nama_barang]" value="${item.nama_barang}" required class="w-full font-bold outline-none mb-1 border-b border-gray-100 text-sm">
                            <textarea name="items[${index}][spesifikasi]" rows="2" class="w-full text-xs text-gray-600 outline-none bg-transparent resize-none border-b border-gray-50 mb-1">${item.spesifikasi || ''}</textarea>
                                    ...
                                    <div class="flex flex-wrap gap-2 mb-2">
                                        ${item.images.map(img => `
                                            <div class="relative group">
                                                <img src="/storage/${img}" class="w-12 h-12 object-cover rounded border border-gray-200">
                                                <input type="hidden" name="items[${index}][existing_images][]" value="${img}">
                                            </div>
                                        `).join('')}
                                    </div>
                                ` : ''}
                                <label class="block text-[9px] uppercase font-bold text-gray-400 mb-0.5">
                                    ${item.images && item.images.length > 0 ? 'Ganti / Tambah Foto' : 'Upload Foto Produk (Opsional)'}
                                </label>
                                <input type="file" name="items[${index}][images][]" multiple accept="image/*" class="w-full text-[10px] text-gray-500 file:mr-2 file:py-0.5 file:px-2 file:rounded file:border-0 file:text-[10px] file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                            </div>
                        </td>
                        <td class="border border-gray-800 px-2 py-1 align-top pt-2">
                            <div class="flex items-center gap-1">
                                <input type="number" name="items[${index}][kuantitas]" value="${item.kuantitas}" step="0.01" required class="w-full text-center outline-none bg-transparent font-medium border-b border-gray-100 text-sm">
                                <input type="text" name="items[${index}][satuan]" value="${item.satuan}" required class="w-12 text-center outline-none bg-transparent text-xs border-b border-gray-100">
                            </div>
                        </td>
                        <td class="border border-gray-800 px-2 py-1 align-top text-right pt-2">
                            <input type="number" name="items[${index}][harga_satuan]" value="${item.harga_satuan}" required class="w-full text-right outline-none bg-transparent font-bold border-b border-gray-100 text-sm">
                        </td>
                        <td class="border border-gray-800 px-1 py-1 text-center align-top pt-2">
                             <button type="button" class="text-red-500 remove-item-edit">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd"></path></svg>
                            </button>
                        </td>
                    </tr>
            ...
            $('#edit-modal-content').html(`
                <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                    <h3 class="text-lg font-bold text-gray-800">Edit SPH: ${data.no_sph}</h3>
                    <button type="button" onclick="closeModal('editModal')" class="text-gray-400 hover:text-gray-600 p-2">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                    </button>
                </div>
                <form action="/admin/surat-penawaran/${data.id}" method="POST" enctype="multipart/form-data" class="flex-1 overflow-y-auto">
                    <input type="hidden" name="_token" value="${$('input[name="_token"]').val()}">
                    <input type="hidden" name="_method" value="PUT">
                    <div class="bg-gray-100 p-4 flex justify-center">
                        <div class="bg-white w-full max-w-[210mm] shadow-lg p-10 text-sm leading-relaxed text-gray-800 border border-gray-200">
                             <!-- Header Section (Raised Date) -->
                            <div class="flex justify-end mb-2">
                                <div class="w-48 text-right">
                                    <label class="block text-[10px] uppercase tracking-wider text-gray-400 font-bold mb-0">Serang, Tanggal</label>
                                    <input type="date" name="tanggal_sph" value="${data.tanggal_sph}" class="w-full border-b border-gray-300 outline-none text-right py-0 font-medium bg-transparent px-0">
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4 mb-4">
                                <div class="space-y-0">
                                    <div class="flex items-start">
                                        <span class="w-16 font-semibold">No.</span>
                                        <span class="mr-2">:</span>
                                        <span class="font-bold">${data.no_sph}</span>
                                    </div>
                                    <div class="flex items-start">
                                        <span class="w-16 font-semibold">Hal.</span>
                                        <span class="mr-2">:</span>
                                        <input type="text" name="perihal" value="${data.perihal}" required class="flex-1 border-b border-gray-200 focus:border-rns-blue outline-none py-0 font-bold underline bg-transparent">
                                    </div>
                                </div>
                            </div>

                            <div class="mb-6 space-y-0 text-sm">
                                <p class="font-bold">Kepada. Yth</p>
                                <input type="text" name="jabatan_customer" value="${data.jabatan_customer || ''}" class="w-full border-b border-gray-200 focus:border-rns-blue outline-none py-0 font-bold bg-transparent">
                                <input type="text" name="nama_customer" value="${data.nama_customer}" required class="w-full border-b border-gray-200 focus:border-rns-blue outline-none py-0 font-bold bg-transparent">
                                <textarea name="alamat_customer" rows="2" class="w-full border-b border-gray-100 focus:border-rns-blue outline-none py-1 bg-transparent resize-none text-xs font-semibold">${data.alamat_customer || ''}</textarea>
                                <input type="hidden" name="no_hp_customer" value="${data.no_hp_customer || ''}">
                            </div>

                            <div class="mb-4">
                                <textarea name="salam_pembuka" rows="3" class="w-full border border-dashed border-gray-200 p-2 rounded focus:border-rns-blue outline-none bg-blue-50/20 text-sm leading-relaxed">${data.salam_pembuka || ''}</textarea>
                            </div>
                            ...
                            <table class="w-full border-collapse border border-gray-800 text-sm mb-4">
                                <thead class="bg-gray-100 uppercase font-bold text-xs">
                                    <tr>
                                        <th class="border border-gray-800 px-2 py-1 w-10 text-center">No</th>
                                        <th class="border border-gray-800 px-2 py-1 text-left">Nama Barang</th>
                                        <th class="border border-gray-800 px-2 py-1 w-24 text-center">Qty</th>
                                        <th class="border border-gray-800 px-2 py-1 w-40 text-center">Harga</th>
                                        <th class="border border-gray-800 px-1 py-1 w-10 text-center"></th>
                                    </tr>
                                </thead>
                                ...
                                <tfoot>
                                    <tr>
                                        <td colspan="3" class="border border-gray-800 px-2 py-1 text-right font-bold uppercase text-xs">Total harga</td>
                                        <td class="border border-gray-800 px-2 py-1 text-right font-bold text-base" id="edit-total-display">0</td>
                                        <td class="border border-gray-800"></td>
                                    </tr>
                                </tfoot>
                            </table>
                            ...
                            <div class="grid grid-cols-1 gap-4 mb-6">
                                <div>
                                    <h4 class="font-bold underline mb-0.5 text-xs">Catatan:</h4>
                                    <textarea name="syarat_ketentuan" rows="4" class="w-full border-0 focus:ring-0 outline-none bg-yellow-50/30 p-2 text-[11px] leading-tight border-l-2 border-yellow-200">${data.syarat_ketentuan || ''}</textarea>
                                </div>
                                <div>
                                    <h4 class="font-bold underline mb-0.5 text-xs">Cara pembayaran:</h4>
                                    <textarea name="keterangan_pembayaran" rows="5" class="w-full border-0 focus:ring-0 outline-none bg-blue-50/20 p-2 text-[11px] leading-tight border-l-2 border-blue-200">${data.keterangan_pembayaran || ''}</textarea>
                                </div>
                            </div>

                            <div class="mb-6">
                                <textarea name="salam_penutup" rows="3" class="w-full border-0 focus:ring-0 outline-none text-xs leading-relaxed bg-transparent">${data.salam_penutup || ''}</textarea>
                            </div>
                            ...
                            <div class="flex justify-end pr-4">
                                <div class="w-64 text-center">
                                    <p class="mb-0.5 text-xs">Hormat kami</p>
                                    <p class="font-bold uppercase text-[10px] mb-1">PT. RANAY NUSANTARA SEJAHTERA</p>
                                    <div class="h-16 flex items-center justify-center italic text-gray-300 border border-dashed border-gray-100 mb-1 text-[10px]">[Tanda Tangan]</div>
                                    <select name="penandatangan" required class="w-full border-b border-gray-200 outline-none font-bold text-center bg-transparent text-xs">
                                        <option value="Heri Pirdaus, S.Tr.Kes Rad" ${data.penandatangan == 'Heri Pirdaus, S.Tr.Kes Rad' ? 'selected' : ''}>Heri Pirdaus, S.Tr.Kes Rad</option>
                                        <option value="Dewi Sulistiowati" ${data.penandatangan == 'Dewi Sulistiowati' ? 'selected' : ''}>Dewi Sulistiowati</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-3 sticky bottom-0 z-10">
                        <button type="button" onclick="closeModal('editModal')" class="px-6 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 shadow-sm font-medium text-sm transition-colors font-bold tracking-wide">BATAL</button>
                        <button type="submit" class="px-8 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 shadow-lg font-bold text-sm transition-all transform hover:scale-105 flex items-center gap-2 tracking-wide uppercase">
                            UPDATE & CETAK
                        </button>
                    </div>
                </form>
            ...
            function calculateTotalEdit() {
                let total = 0;
                $('.item-row-edit').each(function() {
                    const qty = parseFloat($(this).find('input[name*="[kuantitas]"]').val()) || 0;
                    const price = parseFloat($(this).find('input[name*="[harga_satuan]"]').val()) || 0;
                    total += (qty * price);
                });
                $('#edit-total-display').text(new Intl.NumberFormat('id-ID').format(total) + ',-');
            }
            
            $(document).on('input', '.item-row-edit input', calculateTotalEdit);
            calculateTotalEdit();
            
            $(document).on('click', '.remove-item-edit', function() {
                if ($('.item-row-edit').length > 1) {
                    $(this).closest('tr').remove();
                    calculateTotalEdit();
                }
            });
