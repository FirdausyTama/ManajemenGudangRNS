...
<head>
  <meta charset="utf-8" />
  <title>Surat Jalan | RNS - Rand Nusantara Sejahtera</title>
  ...
  <main id="main-content" class="flex-1 md:ml-64 pt-16 flex flex-col h-screen overflow-x-hidden relative transition-all duration-300">
    <div class="flex-1 overflow-x-hidden overflow-y-auto p-4 md:p-8 relative w-full">
        ...
        <div class="max-w-7xl mx-auto relative z-10">
          ...
          <!-- Search & Filter Card -->
          <form action="{{ route('surat-jalan.index') }}" method="GET" class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row md:flex-wrap items-center gap-4 mb-6">
                          ...
                          <tr class="hover:bg-gray-50/50 transition-colors">
                              ...
                              <td class="py-3 px-4 text-center">
                                  <div class="flex items-center justify-center gap-2">
                                      ...
                                      <form action="{{ route('surat-jalan.destroy', $inv->id) }}" method="POST" id="delete-form-{{ $inv->id }}" class="inline">
                                          @csrf @method('DELETE')
                                          <button type="button" onclick="confirmDelete('{{ $inv->id }}')" class="p-1.5 text-red-600 hover:bg-red-50 rounded" title="Hapus Surat Jalan">
            ...
            <form id="addForm" action="{{ route('surat-jalan.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="date" name="tanggal_surat_jalan" value="{{ date('Y-m-d') }}" max="{{ date('Y-m-d') }}" required class="w-full rounded-lg border-gray-300 border px-4 py-2 text-sm focus:ring-rns-blue focus:border-rns-blue bg-white">
                    </div>
                </div>
  ...
  <script>
    ...
    function confirmDelete(id) {
        Swal.fire({
            ...
            text: 'Yakin ingin menghapus Surat Jalan ini?',
            icon: 'warning',
