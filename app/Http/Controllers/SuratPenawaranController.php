...
class SuratPenawaranController
{
    ...
    public function update(Request $request, SuratPenawaran $surat_penawaran)
    {
        $request->validate([
            'nama_customer' => 'required|string',
            'jabatan_customer' => 'nullable|string',
        ...
        try {
            ...
            // Delete existing items and their images if necessary (or keep them)
            // For simplicity, we'll keep the logic of deleting and recreating, 
            // but we need to pass existing image paths back if not re-uploading.
            // However, a better way is to update. I'll stick to a slightly more robust recreation.
            
            $existingItems = $surat_penawaran->items;
            $surat_penawaran->items()->delete();
            ...
            foreach ($request->items as $index => $itemData) {
                ...
                } else if (isset($itemData['existing_images'])) {
                    // Restore existing images if they were passed back
                    $itemImages = is_array($itemData['existing_images']) ? $itemData['existing_images'] : json_decode($itemData['existing_images'], true);
                }

                SuratPenawaranItem::create([
                    'surat_penawaran_id' => $surat_penawaran->id,
                    'barang_id' => $itemData['barang_id'] ?? null,
                    'nama_barang' => $itemData['nama_barang'],
                    'spesifikasi' => $itemData['spesifikasi'],
                    'kuantitas' => $itemData['kuantitas'],
            ...
            $surat_penawaran->update(['total_harga' => $total_harga]);

            DB::commit();
            return back()->with('success', 'Surat Penawaran berhasil diperbarui.');
        } catch (\Exception $e) {
            DB::rollBack();
            return back()->with('error', 'Gagal memperbarui Surat Penawaran: ' . $e->getMessage());
        }
    }
